...
if [ -e "$SQUASHED" ]; then
   ...
   # elif [ -f /run/initramfs/squashfs/LiveOS/ext3fs.img ]; then
    ...
    #fi
    #mksquashfs yaparken LiveOS içine atmaya gerek yok imajları
    if [ -f /run/initramfs/squashfs/rootfs.img ]; then
        FSIMG="/run/initramfs/squashfs/rootfs.img"
    elif [ -f /run/initramfs/squashfs/ext3fs.img ]; then
        FSIMG="/run/initramfs/squashfs/ext3fs.img"
    fi
